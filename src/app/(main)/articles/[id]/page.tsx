...
export default async function ArticleDetailPage({ params }: ArticleDetailPageProps) {
    ...
    const articleText = typeof article.text === "string" ? article.text : article.text ? JSON.stringify(article.text) : ""
                ...
                {articleContent.length > 0 ? (
                    articleContent.map((block, index) => {
                        switch (block.type) {
                            case "section":
                                return (
                                    <section key={`section-${block.id}`} id={block.id} className="mb-8">
                                        <h2 className="mds:text-2xl text-[20px] sml:font-semibold font-medium text-gray-900 mb-4">{block.title}</h2>
                            ...
                            case "product":
                                return (
                                    <div key={`product-${index}`} className="mt-6 mb-4 border sml:rounded-[16px] rounded-[8px] sml:p-4 p-2 bg-[#F5F7FF]">
                                        <div className="flex justify-between items-center">
                                            <div className="flex sml:gap-4 gap-3 items-center">
                                                ...
                                                <div>
                                                    <h3 className="font-medium sml:text-[18px] text-[15px] text-[#333438]">{block.content?.name}</h3>
                                                </div>
                                            </div>
                                            <div
