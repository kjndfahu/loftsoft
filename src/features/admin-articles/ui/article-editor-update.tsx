...
import { ImageIcon, Save, ArrowLeft, Trash2, AlertTriangle } from "lucide-react"
import { updateArticle, deleteArticle, uploadArticleMedia } from "@/enteties/articles/article"
...
const Modal = ({ isOpen, onClose, onConfirm, title, description, confirmText, cancelText }: ModalProps) => {
...
export const ArticleEditor = ({ article }: ArticleEditorProps) => {
    ...
    const [mainImage, setMainImage] = useState<string | null>(article.photo || null)
    const [mainImageFile, setMainImageFile] = useState<File | null>(null)
    const [content, setContent] = useState("")
    const [selectedProducts, setSelectedProducts] = useState<ArticleContentBlock[]>([])
    ...
    // Initialize content and selected products from article.text
    useEffect(() => {
        try {
            const parsedContent = JSON.parse(article.text || "[]") as ArticleContentBlock[]
            const textBlocks = parsedContent
                .filter((block) => block.type !== "product")
                .map((block) => {
                    if (block.type === "text") return `<p>${block.content}</p>`
                    if (block.type === "image") return `<img src="${block.content}" alt="${block.caption || ""}" />`
                    if (block.type === "quote") return `<blockquote>${block.content.text}</blockquote>`
                    if (block.type === "section") return `<h2>${block.title}</h2><p>${block.content}</p>`
                    if (block.type === "link") return `<p><a href="${block.content.url}">${block.content.title}</a></p>`
                    if (block.type === "video") return `<p>Video: ${block.content.url}</p>` // Simplified for Quill
                    if (block.type === "relatedArticle") return `<p>Related Article: ${block.content.title}</p>`
                    return ""
                })
                .join("")
            setContent(textBlocks)
            const productBlocks = parsedContent.filter((block) => block.type === "product")
            setSelectedProducts(productBlocks)
        } catch (e) {
    ...
    const handleMainImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            try {
                ...
                setMainImage(article.photo || null)
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            let mainImageUrl = article.photo || ""
            if (mainImageFile) {
                const formData = new FormData()
            ...
            alert(`Error updating article: ${error instanceof Error ? error.message : String(error)}`)
        } finally {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            alert(`Error deleting article: ${error instanceof Error ? error.message : String(error)}`)
        } finally {
            setIsLoading(false)
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
            <div className="flex items-center text-black gap-4 mb-6">
                ...
                <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2 bg-black text-white rounded-md hover:bg-gray-800 transition-colors ${isSubmitting || !title ? "opacity-50 cursor-not-allowed" : ""}`}
                >
                ...
                <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-red-500 text-white rounded-md hover:bg-red-600 transition-colors ml-auto"
                >
                    <Trash2 size={16} />
                    <p className="sml:flex hidden">Удалить статью</p>
                </button>
                <Modal
            ...
            <div className="space-y-4 text-black">
                ...
                <div>
                    ...
                    <div
                        className={`relative h-[250px] rounded-[40px] overflow-hidden ${mainImage ? "" : "bg-[#B9BCCB]"} cursor-pointer transition-all duration-200 ${isHoveringMainImage ? "bg-[#A4A8BA]" : ""} flex items-center justify-center`}
