...
export const CartItemsList = ({ onSelectionChange, selectedItems, setSelectedItems }: {
    ...
    const handleCheckboxChange = (itemId: string, checked: boolean) => {
        ...
        if (checked) {
            ...
            updatedSelection = selectedItems.filter((id) => id !== itemId);
        }
        ...
        <div className="flex flex-col w-full gap-[20px]">
            {Object.entries(groupedItems).map(([type, typeItems]) => (
                <div key={type} className="flex flex-col py-5 border-t-[1px] last:border-b-[1px] gap-[20px]">
                    ...
                    {typeItems.map((item) => (
                        <div key={item.id} className="flex items-start w-full md:gap-[80px] gap-5 justify-between">
                            <CartItemInfo
                                name={item.name}
                                photo={item.photo}
                                licenseType={`${item.deviceCount} ПК`}
                                isChecked={selectedItems.includes(item.id)}
                                onCheckboxChange={(checked) => handleCheckboxChange(item.id, checked)}
                            />
