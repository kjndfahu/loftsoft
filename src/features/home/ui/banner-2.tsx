            ...
            <div className="relative w-full h-full overflow-hidden">
                ...
                >
                    {banners.map((banner, index) => (
                        <div
                            key={index}
                            className={`min-w-full ${banner.background} bg-cover bg-no-repeat sm:bg-[position:0px]`}
                        >
                            <div className="flex flex-col cursor-pointer gap-4 mds:p-10 p-6 sm:h-[536px] h-[240px] rounded-[20px]">
                                <h2 className="text-[#424141] md:text-[34px] text-[24px] md:leading-[40px] leading-[28px] font-semibold">
                                    {banner.title.split(" ").map((word, idx) => (
                                        <span key={idx}>
                                            {word}
                                            {idx < banner.title.split(" ").length - 1 && " "}
                                            {idx === 1 && <br />}
                                        </span>
                                    ))}
                                </h2>
                                <p className={`md:text-[16px] w-[${banner.descWidth}] text-[14px] md:leading-6 leading-4 text-[#343434]`}>
                                    {banner.description.split(". ").map((sentence, idx) => (
                                        <span key={idx}>
                                            {sentence}
                                        </span>
                                    ))}
                                ...
                                <div className="absolute sm:flex hidden items-end justify-end w-[237px] h-[241px] right-0 bottom-0 bg-white rounded-tl-[22px]">
                                    <div className="flex flex-col text-white items-center justify-between p-5 rounded-[20px] w-[217px] h-[221px] bg-[linear-gradient(126.44deg,_#516DEB_-1.33%,_#D6DDFF_100%)]">
                                        <p className="font-medium text-[14px]">{banner.statsDescription}</p>
                                        <h3 className="self-start font-semibold leading-[50px] text-[50px]">{banner.stats}</h3>
                                    </div>
                                </div>
                            </div>
                        </div>
