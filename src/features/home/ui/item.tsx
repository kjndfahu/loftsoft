...
import type { Product } from "@/features/home/ui/items-grid"
...
export const Items = ({ product }: ItemsProps) => {
    if (!product) {
        return (
            <div className="flex flex-col w-full mds:gap-6 gap-[10px]">
                <div style={{ aspectRatio: "312/415" }} className="w-full bg-[#F5F7FF] rounded-[16px] relative overflow-hidden">
    ...
    const renderImage = () => {
        ...
        if (isBase64Image(product.photo)) {
            return <img src={product.photo || "/placeholder.svg"} alt={product.name} className="object-cover w-full h-full" />
        } else {
            return (
                <Image
                    src={
    ...
    return (
        <Link href={`/catalog/${product.id || ""}`} className="flex flex-col w-full mds:gap-6 gap-[10px]">
            <div style={{ aspectRatio: "312/415" }} className="w-full bg-[#F5F7FF] rounded-[16px] relative overflow-hidden">
                {renderImage()}
            </div>
            <div className="flex flex-col text-[#161616] sm:gap-[10px] gap-2">
                <h3 className="mds:text-[27px] sm:text-[20px] text-[16px] font-semibold">{formatPrice(product.price)} ₽</h3>
                <div className="flex font-medium mds:text-[16px] sm:text-[14px] text-[12px] gap-1">
                    ...
                    <div className="flex bg-[#ACB1C626] rounded-[6px] text-[#161616] py-[4px] px-[6px]">
                        {Math.floor(Math.random() * 500) + 100} купили
                    </div>
                </div>
                <p className="mds:text-[20px] sm:text-[17px] text-[14px] font-medium">{product.name}</p>
            </div>
        </Link>
    )
}
