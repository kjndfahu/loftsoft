    ...
    return (
        <div className="flex h-full w-[312px] flex-col gap-[12px] rounded-[16px] border-[1px] border-[#E9EBF6] p-3">
            ...
            <div className="flex flex-col">
                <div onClick={() => setTab('profile')} className={`flex items-center cursor-pointer ${tab === 'profile' ? 'bg-[#F5F7FF]' : 'bg-white'} gap-[6px] rounded-[8px] py-[11px] px-[16px]`}>
                    <UserLogo/>
                    <h4 className={`${tab === 'profile' ? 'text-[#5069E8]' : 'text-[#161616]'} text-[14px]`}>Профиль</h4>
                </div>
                <div onClick={() => setTab('orders')} className={`flex items-center cursor-pointer ${tab === 'orders' ? 'bg-[#F5F7FF]' : 'bg-white'} gap-[6px] rounded-[8px] py-[11px] px-[16px]`}>
                    <BoxLogo/>
